...
''' True field ''' 
s_true = np.loadtxt('./Materials/Ref_Gaussian.txt')
s_true = s_true*2-1
x = np.linspace(0. + dx[0] / 2., Lx - dx[0] / 2., N[0])
y = np.linspace(0. + dx[1] / 2., Ly - dx[1] / 2., N[1])
...
netG = netG()
netG.load_state_dict(torch.load('./materials/Weights_Gaussian.pth',map_location='cpu'))
netG.to(device)
netG.eval()
...
R = 0.02**2
invR = 1./R
obs = obs_true + 0.02*np.random.randn(len(obs_true),1)
...
def plot_func(z,Q_locs,i=0):
  ...
  for Q_loc in Q_locs:                                       # the location of wells
      ax[0].scatter(Q_loc[1],Q_loc[2], marker='o',c='None', edgecolors='r', s = 20)
